...
SELECT sb.Name AS SubjectName, COUNT(*) AS Count
...
SELECT st.Name, sb.Name, AVG(sm.Mark) AS Average
FROM StudentMarks sm
INNER JOIN Students st
ON st.Id = sm.StudentId 
INNER JOIN Subjects sb
ON sm.SubjectId = sb.Id
GROUP BY sm.StudentId, sm.SubjectId;

s
SELECT sb.Name
FROM StudentMarks sm
RIGHT JOIN Subjects sb
ON sb.Id = sm.SubjectId
WHERE sm.Mark IS NULL;

SELECT * FROM Students st
ORDER BY st.ClassNum, st.ClassLetter, st.Name;

SELECT sb.Name, AVG(sm.Mark) as Average
FROM Subjects sb
INNER JOIN StudentMarks sm
ON sb.Id = sm.SubjectId
GROUP BY sb.Name
HAVING AVG(sm.Mark) < 2.50
ORDER BY Average;